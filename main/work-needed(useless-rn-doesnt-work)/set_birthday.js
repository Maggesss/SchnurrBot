        ...
        try {
            ...
            if (fs.existsSync(path.resolve(`./data/server/${interaction.guild.id}/birthdays/`))) {
                if (!fs.existsSync(path.resolve(`./data/server/${interaction.guild.id}/birthdays/${interaction.user.id}`))) {
                    if (!(functions.isInt(day) && functions.isInt(month))) { return interaction.reply({ content: `Your input isn't right!`, ephemeral: true }) }
                    ...
                    if ((month == 2) && (day > 29)) { return interaction.reply({ content: `February doesn't have more than 29 Days.`, ephemeral: true }) }
